...
void compute_sine_wave()
{
	...
	double win_t = 5*mySineWave.UR/1000; //window size, 10 ms window를 반 나누어 앞과 뒤에 씌움. 5 ms.
	for ( i=0 ; i<ulCount ; i++ ) {
		if(i<win_t) { //Window 씌우기
			pdOutputData[i]= mySineWave.A *0.5*(1-cos(2*PI*i/(win_t*2-1))) * sin( 2*PI*mySineWave.F*mySineWave.del_t*i );
		}
		else if(i>(ulCount-win_t)) {
			pdOutputData[i]= mySineWave.A *0.5*(1-cos(2*PI*(ulCount-i)/(win_t*2-1))) * sin( 2*PI*mySineWave.F*mySineWave.del_t*i );
		}
		else
		{
