...
#define DAQmx_DO_Tristate                                                0x18F3 // Specifies whether to stop driving the channel and set it to a Hi-Z state. You must commit the task for this setting to take effect.
...
#define DAQmx_Exported_HshkEvent_OutputBehavior                          0x22BB // Specifies the output behavior of the Handshake Event. If the event is interlocked, the  If the event is pulse, the event pulses for Event:Handshake Event:Pulse:Pulse Width seconds.
...
#define DAQmx_Hshk_StartCond                                             0x22C3 // Specifies at which point in the handshake cycle the device is in when the task starts.                                                .the initial state of the hardware: Immediate, Wait for Handshake Trigger to Assert, or Wait for Handshake Trigger to Deassert. Immediate means the device is waiting for space in the FIFO (for acquisition) or waiting for data (for generation).
